...
export default class Role extends Component {
  ...
  addRole = () => {
    ...
    form.validateFields(async (err,values) => {
      if(!err){
        // console.log(values);
        const result = await reqAddRoles(values.name);
        if(result){
  ...
  //设置角色权限的回调函数
  updateRole = async () => {
    // console.log(this.updateRoleForm.state)
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render:time => dayjs(time).format("HH-MM-DD HH:mm:ss")
      },
      {
        title: '授权时间',
        dataIndex: 'auth_time',
    ...
    return (
      ...
      >
        ...
        <Modal
          ...
          cancelText='取消'
        >
          <UpdateRoleForm wrappedComponentRef={(form) => this.updateRoleForm = form} roleName={role ? role.name : ''}/>
        </Modal>
